...
void update_GUI_strings(State& state)
{
    auto& values = state.m_textValues;

    int ss = static_cast<int>(state.m_time) % 60;
    int mm = static_cast<int>(state.m_time) / 60;
    values.time.setString(std::format("{:02d}:{:02d}", mm, ss));

    values.alt.setString(std::format("{:04d}", (int)state.m_ship.position.y));
    values.pos.setString(std::format("{:04d}", (int)state.m_ship.position.x));
    values.fuel.setString(std::format("{:04d}", state.m_ship.fuel));
    values.horzSpeed.setString(std::format("{:d}", (int)state.m_ship.velocity.x));
    values.vertSpeed.setString(std::format("{:d}", (int)state.m_ship.velocity.y));
}

void init(State& state, Level gameLevel)
{
    ...
    // Load font
    if (!state.m_uMono.loadFromFile("../resources/UbuntuMono-Regular.ttf"))
    {
        std::cout << "Couldn't find font file!\n";
    }
    // Create text objects
    auto create_text = [&state](const char* str, sf::Vector2f pos)
    {
        sf::Text text;
        text.setFont(state.m_uMono);
    ...
    // Labels
    state.m_textLabels.time = create_text("TIME", {.2, .05});
    state.m_textLabels.alt = create_text("ALTITUDE", {.2, .1});
    state.m_textLabels.pos = create_text("POSITION", {.2, .15});
    state.m_textLabels.fuel = create_text("FUEL", {.7, .05});
    state.m_textLabels.horzSpeed = create_text("HORIZONTAL SPEED", {.7, .1});
    state.m_textLabels.vertSpeed = create_text("VERTICAL SPEED", {.7, .15});
    // Values
    state.m_textValues.time = create_text("00:00", {.3, .05});
    state.m_textValues.alt = create_text("0000", {.3, .1});
    state.m_textValues.pos = create_text("0000", {.3, .15});
    state.m_textValues.fuel = create_text("0000", {.8, .05});
    state.m_textValues.horzSpeed = create_text("0", {.8, .1});
    state.m_textValues.vertSpeed = create_text("0", {.8, .15});
    // Update strings so that it looks correct on the first frame
    update_GUI_strings(state);

    // Win/Loss messages
    state.m_winMsg = create_text("SUCCESS!", {0.5, 0.3});
    state.m_winMsg.setCharacterSize(64);
    sf::FloatRect bounds = state.m_winMsg.getLocalBounds();
    state.m_winMsg.setOrigin(bounds.getSize() / 2.0f);

    state.m_loseMsg = create_text("BOOM", {0.5, 0.3});
    state.m_loseMsg.setCharacterSize(64);
    bounds = state.m_loseMsg.getLocalBounds();
    state.m_loseMsg.setOrigin(bounds.getSize() / 2.0f);
}
...
void physics(State& state, float dt)
{
    ...
    bool isSimulation = !state.m_window.isOpen();
    ...
    // Update GUI strings (only if the game is being rendered to a window)
    if (state.m_window.isOpen())
    {
        update_GUI_strings(state);
    }
}
...
void draw_scene(sf::RenderWindow& win, State const& state)
{
    win.clear();
    ...
    // Draw GUI
    auto& txtLabels = state.m_textLabels;
    auto& txtValues = state.m_textValues;
    ...
    case State::Stage::Won:
        win.draw(state.m_winMsg);
        break;
    case State::Stage::Lost:
        win.draw(state.m_loseMsg);
        break;
...
void run(State& state, GameSpeed speed)
{
    //double t = 0.0;
    sf::Clock clock;
    ...
    while (state.m_window.isOpen())
    {
        sf::Event event;
        while (state.m_window.pollEvent(event))
        {
            if (event.type == sf::Event::Closed)
            {
                state.m_window.close();
            }
        }
        ...
        draw_scene(state.m_window, state);
    }
}
